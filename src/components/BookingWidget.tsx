...
import { Search, MapPin, Calendar as CalendarIcon, Users, ChevronDown, ChevronRight, Plus, Minus } from "lucide-react";
  ...
  return (
    <div className="relative w-full max-w-4xl mx-auto z-40">
      ...
      >
        ...
        {/* SEARCH BUTTON */}
        <Link href="/search" className="block w-full md:w-auto mt-2 md:mt-0 ms-0 md:ms-2">
          <button className="w-full h-full flex items-center justify-center gap-2 bg-primary hover:brightness-110 text-white px-8 py-5 md:py-4 rounded-full font-bold text-sm transition-all shadow-md">
      ...
      {activeTab === "dates" && (
        <div className="absolute top-[110%] left-0 md:left-1/4 w-[100%] md:w-auto bg-surface rounded-2xl p-6 z-50 flex flex-col items-center border border-navy/5" style={{ boxShadow: "var(--shadow-hover)" }}>
          <DayPicker
            mode="range"
          ...
          <div className="flex items-center justify-between py-4 border-b border-navy/5">
            ...
            <div className="flex items-center gap-4">
              <button 
                ...
                className="w-8 h-8 rounded-full border border-navy/20 flex items-center justify-center text-navy hover:border-navy transition-colors disabled:opacity-30 disabled:hover:border-navy/20"
                disabled={adults <= 1}
              >
