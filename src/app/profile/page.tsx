...
import { useUser as useClerkUser, SignOutButton, useAuth } from "@clerk/nextjs";
...
export default function ProfilePage() {
  ...
  useEffect(() => {
    const fetchLiveData = async () => {
      ...
      const [bData, pData] = await Promise.all([
        getBookings(token || undefined, clerkUser?.primaryEmailAddress?.emailAddress),
        getProperties()
      ]);
    ...
    if (isLoaded && clerkUser) {
      ...
      const channel = supabase
        ...
        .on(
          'postgres_changes',
          { event: '*', schema: 'public', table: 'bookings', filter: `guest_email=eq.${clerkUser.primaryEmailAddress?.emailAddress}` },
          () => {
  ...
  const isWithinTwoHours = (createdAt?: string | null) => {
    if (!createdAt) return false;
    return (new Date().getTime() - new Date(createdAt).getTime()) < 7200000; // 2 hours in ms
  };
              ...
              {bookingFilter === "upcoming" && (() => {
                ...
                return (
                    ...
                    <div className="flex items-start gap-5 text-start">
                      ...
                      <div className="space-y-1">
                        ...
                        <p className="text-navy/60 text-sm max-w-xl leading-relaxed">
                          Your booking for <span className="font-bold text-navy">{activeFailedBooking.property.title}</span> was not completed. We have held your dates, but action is needed to secure them.
                        </p>
                      </div>
                    </div>
                  <div className="flex flex-col sm:flex-row items-center gap-3 w-full md:w-auto">
                    ...
                    <Link
                      href={`/checkout?id=${activeFailedBooking.property.id}&adults=${activeFailedBooking.adults || 2}&children=${activeFailedBooking.children || 0}&from=${activeFailedBooking.checkIn}&to=${activeFailedBooking.checkOut}`}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  ...
                  <div className="col-span-1 md:col-span-2 flex justify-end mt-4">
                    <button
                      ...
                      className={`px-8 py-3 rounded-xl text-xs font-black uppercase tracking-[0.2em] transition-all flex items-center gap-3 ${isSaved ? 'bg-emerald-500 text-white' :
