...
import { DollarSign, TrendingUp, CreditCard, Download, Loader2, ArrowUpRight, Wallet, LayoutGrid, Store } from "lucide-react";
...
export default function FinancialsDashboard() {
  ...
  // ── THE ARCHITECT'S TIME-AWARE LOGIC ──
  const now = new Date().getTime();

  // 1. Pure Revenue (For Top Cards & CSV Export)
  const confirmedBookings = bookings.filter(b => b.status === "confirmed" || b.payment_status === "paid");

  // 2. Lost Opportunities (Last 24 Hours Only)
  const failedBookings = bookings.filter(b => {
    if (b.payment_status !== "failed") return false;
    const hoursAgo = b.created_at ? Math.floor((now - new Date(b.created_at).getTime()) / 3600000) : 0;
    return hoursAgo <= 24;
  });

  // 3. Table Records (Confirmed + Active Primary Leads + Active Secondary Leads)
  ...
  const handleExportCSV = () => {
    ...
    // Export ONLY confirmed revenue to accounting CSV
        ...
        <>
          ...
          {failedBookings.length > 0 && (
            <div className="bg-rose-50 border border-rose-100 rounded-3xl p-6 flex flex-col md:flex-row items-center justify-between gap-6 shadow-sm">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="text-xs font-black uppercase tracking-widest text-rose-600">Lost Opportunity (Last 24h)</p>
              ...
              <div className="text-center md:text-right">
                <p className="text-sm text-navy/60 max-w-xs">
                  You have <span className="font-bold text-rose-600">{failedBookings.length} recent failed attempts</span>. Recovering just 20% of these could add <strong>${Math.round(potentialRevenue * 0.2).toLocaleString()}</strong> to your net payout.
                </p>
              </div>
          ...
          <div className="bg-white rounded-2xl shadow-soft border border-navy/5 overflow-hidden">
            <div className="p-4 md:p-6 border-b border-navy/5 flex items-center justify-between">
              ...
              <span className="text-[10px] md:text-xs text-muted font-bold uppercase tracking-widest">{tableBookings.length} ACTIVE RECORDS</span>
            </div>
              ...
              <>
                <div className="hidden md:block overflow-x-auto">
                  <table className="w-full text-left border-collapse">
                    ...
                    <tbody className="divide-y divide-navy/5">
                      {tableBookings.sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime()).map((booking) => {
                        const net = isWhiteLabel ? booking.total_price : booking.total_price * 0.9;
                        const isPrimaryLead = booking.payment_status === 'failed';
                        const isSecondaryLead = booking.status === 'pending' && booking.payment_status !== 'failed';
                        const isConfirmed = booking.status === 'confirmed' || booking.payment_status === 'paid';

                        return (
                          <tr key={booking.id} className={`hover:bg-slate-50/40 transition-colors group ${isPrimaryLead ? "bg-rose-50/10" : isSecondaryLead ? "bg-amber-50/10" : ""}`}>
                            ...
                            <td className="px-6 py-4">
                              <span className="text-xs font-mono font-bold text-navy/40">
                                {(isPrimaryLead || isSecondaryLead) ? "LEAD-" : ""}{booking.booking_reference || `TX-VST-${booking.id.toString().padStart(4, "0")}`}
                              </span>
                            </td>
                            <td className="px-6 py-4">
                              <div className="flex items-center gap-3">
                                ...
                                {isPrimaryLead && <span className="text-[8px] font-black uppercase px-1.5 py-0.5 bg-rose-50 text-rose-600 border border-rose-100 rounded">Lead</span>}
                                {isSecondaryLead && <span className="text-[8px] font-black uppercase px-1.5 py-0.5 bg-amber-50 text-amber-600 border border-amber-100 rounded">Lead</span>}
                              </div>
                              <p className="text-xs text-muted mt-0.5">{booking.guest_name}</p>
                            </td>
                            <td className="px-6 py-4 text-right">
                              <span className={`font-bold text-sm ${(isPrimaryLead || isSecondaryLead) ? "text-navy/30 line-through" : "text-navy"}`}>${booking.total_price.toLocaleString()}</span>
                            </td>
                            <td className="px-6 py-4 text-right">
                              {isPrimaryLead ? (
                                <span className="text-[10px] font-black text-rose-500 uppercase tracking-tighter animate-pulse">Interrupted</span>
                              ) : isSecondaryLead ? (
                ...
                <div className="md:hidden divide-y divide-navy/5">
                  {tableBookings.map((booking) => {
                    ...
                    return (
                      <div key={booking.id} className={`p-4 flex flex-col gap-3 ${isPrimaryLead ? "bg-rose-50/10" : isSecondaryLead ? "bg-amber-50/10" : ""}`}>
                        ...
                        <div className="flex justify-between items-start">
                          ...
                          <div className="text-right">
                            <p className={`text-xs font-bold ${(isPrimaryLead || isSecondaryLead) ? "text-navy/30 line-through" : "text-navy"}`}>${booking.total_price.toLocaleString()}</p>
                            {isPrimaryLead ? (
                              <p className="text-[10px] font-black text-rose-500 mt-1 uppercase animate-pulse">Interrupted</p>
                            ) : isSecondaryLead ? (
                              <p className="text-[10px] font-black text-amber-500 mt-1 uppercase">Pending</p>
                            ) : (
                              <p className="text-[10px] font-black text-emerald-600 mt-1">NET +${net.toLocaleString()}</p>
                            )}
                          </div>
                        </div>
                      </div>
